...
export function start(detail){   
    ...
    detail.inf    = {entity: 'customer', report:'account-receivable-all',  }

    reportDB.select(detail, phpFile, (data)=>{ //console.log('accountReceivable ALLLLLLLLL>', data) 
...
function generateHeader(){
   ...
   var header = ''
        header +=`<div class="report-page" id="miDiv5" ${css.table}>`
            ...
            header +=`
                    <div class="dataCont">
                        <h1  ${css.f32}>Accounts Receivable Aging Report  </h1>
                        <p   ${css.f14} >
                                This is a financial document that provides information about money 
...
function generateTop(data){console.log('generateTop>>>', data)
        ...
        let detailHtml = `data-detail='{"click": [ {"dest":"ripple" },  {"dest":"report", "act":"jump"  } ] }'`
            ...
            table += `
                ...
                <tr>
                  <td ${css.td0} ${detailHtml} ><b>${data.customer_id}     </b></td>
...
function generateTable(data){ //console.log('data>>>', data) 
    ...
    var table = `
    <h2 ${css.tableTitle}>Customers Vs Aging Owned Amounts</h2>
    ...
    for ( let i=0; i< data.length; i++ ){ //console.log('for>>>', data.length)
        ...
        {"dest":"customer", "elem":"report", "reportType":"accountReceivable", "entity":"customer", "dbid":"${data[i].customer_id}", "source":"dt" } ] }'`            
        ...
        table += `
            <tr ${rowStyle(i)}>
              ...
              <td class='tdClick' ${css.td} ${detailHtml} >${data[i].customer}</td>
