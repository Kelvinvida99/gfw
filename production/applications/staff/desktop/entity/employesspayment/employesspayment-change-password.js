...
const employesspaymentSnack         = require('./employesspayment-snack')
...
export const start = (detail) => { console.log('change password', detail)
    ...
    return changeDB(detail)
      .then((result)=>  { 
        ...
        employesspaymentSnack.start({act:'show', id:'passwordUpdated'})

      }).catch((error) => { //console.log('Settings load notification', error)

        employesspaymentSnack.start({act:'show', id:'passwordUpdateError'})
