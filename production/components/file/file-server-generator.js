...
export function printFile(obj){  //console.log('printingFile>') 
  ...
  */
  ...
  var knownFileExt = {
    ...
    if(file.folder == "true"){
      ...
      fileHmtl += `<div class="cont ${file.emptyClass}"  data-fileid = "${file.id || 0}">
                      ...
                      <div class="des"  >
                        <div class="img"> </div>
                        <label class="fileFolderLabel">${file.name}</label>
                      </div> 
      ...
      if(typeof knownFileExt[fileExt] !== 'undefined') {
        ...
        if(fileExt == "cont-img"){
          ...
          imgIconPath = `style="background: url('./server/storage/beestock/entities/${obj.entity}/${obj.id}/uploaded_files/${obj.currentPath}/lowlowlowCompression/${file.name}') !important;"`;     
      ...
      fileHmtl += `<div class="cont ${fileExt}"   data-fileid = "${file.id || 0}">
                      ...
                      <div class="img" ${imgIconPath}  data-detail='{"click": [ {"dest":"file", "act": "viewFile", "entity":"${obj.entity}", "file":"${file.name}", "fileExt":"${fileExt}"} ] }'> </div>
                      
                      <div class="check" data-detail='{"click": [ {"dest":"file", "act": "checking" } ] }'>
                        <svg><use xlink:href="./css/svg.svg#check-round"></use></svg>
                      </div>   

                      <div class="des" data-detail='{"click": [ {"dest":"file", "act": "viewFile", "entity":"${obj.entity}", "file":"${file.name}", "fileExt":"${fileExt}"} ] }' >
                        <div class="img"> </div>
                        <label class="fileFolderLabel">${file.name}</label>
                      </div> 
  ...
  if(fileHmtl != ""){
    ...
    fileHmtl =  `<div id="b" style="z-index:1;" class="cont-empty cont-empty-upload" data-detail='{"click": [ {"dest":"ripple"}, {"dest":"file", "act":"triggerFileUpload", "formId":"${obj.entity}-fullcont"} ], "drop": [ {"dest":"file", "act":"dropEvent", "entity":"${obj.entity}"} ] }'>
