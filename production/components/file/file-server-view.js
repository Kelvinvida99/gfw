...
export function start(detail){
 ...
 let fileFormat = detail.file.split(".");
...
if(files.length == 1 && fileFullViewDiv.querySelector('h1').innerHTML == files[pos]){//DONT CHANGE SOURCE IF ONLY 1 FILE CAN BE VISIBLE
    return;
  }
...
export function resizeFileView(detail){
    const iframe = document.querySelector('#file_full').querySelector('iframe');
...
function open(){ 
  ...
  const scrimClass  = ["scrim", "scrim-show"]
    ...
    backControl.hashAdd({ toShow:`file_full` })
