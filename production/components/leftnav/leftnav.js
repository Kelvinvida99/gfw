...
export function selectById(id){  console.log('leftnav selectById', leftnav) 

  if(!HTMLwasSelected) { HTMLselect() }

	const elem    = document.getElementById(id)
	const elemAll = leftnav.querySelectorAll('.cont')

	for (let i = 0; i < elemAll.length; i++) { 
			elemAll[i].classList.remove('select')
	}

	if( elem != undefined ){ elem.classList.add('select') }
