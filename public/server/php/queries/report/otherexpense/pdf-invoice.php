...
$_POST["entity"] = "otherexpense";//COMMENT
...
foreach ($rows as $row) {
    ...
    $fax           = $row["provider_fax"];

    //VENDOR
    $vendor       = new stdClass();
    $vendor->name = $row["provider_name"];
    ...
    $expense_id                  = json_decode($row['provider_id'], true);
    $notes                       = $row['notes'];
    $amount                      = $row['amount'];
    $accounts_expense_name       = $row['accounts_expense_name'];
    $item                        = new stdClass(); // Crear un nuevo objeto para cada elemento
    $item->name                  = $expense_id[0]['displayText'];
    $item->notes                 = $notes;
    $item->amount                = $amount;
    $item->accounts_expense_name = $accounts_expense_name;
...
//         $expense_id         = json_decode($elem['expense_id'], true);
...
$pdf->Cell(43,5,'EXPENSE',0,0,'L',1);
$pdf->Cell(30,5,'TYPE',0,0,'L',1);
$pdf->Cell(60,5,'DESC',0,0,'L',1);
$pdf->Cell(30,5,'AMOUNT',0,0,'L',0);
...
$pdf->Cell(43, 6, $item->name, 0, 0, '', 1);
$pdf->Cell(30, 6, $item->accounts_expense_name, 0, 0,'', 1);
$pdf->Cell(60, 6, $item->notes, 0, 0, '', 1);
$pdf->Cell(30, 6, "$".number_format($item->amount,2), 0, 0, '', 1);
...
$file_name = "../../../../storage/beestock/entities/otherexpense/sent-invoice/f-other_id-".rand ( 89 , 9999999 ).".pdf";
