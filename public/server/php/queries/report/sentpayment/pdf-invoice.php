...
$_POST["entity"] = "sentpayment";//COMMENT
...
foreach ($rows as $row) {
    ...
    $code         = $row["code"];
    $po_purchase  = $row["purchase_code"];
    $po_provider  = $row["provider_code"];
    $po = "";
...
foreach ($invoice_item->items as $key => $elem) {
    if ($key !== 0) {
        ...
        $pdf->SetFont('Arial','',8);  //$pdf->Cell(50);

        $pdf->Cell(30, 6, $elem->name, 0, 0, '', 1);
        $pdf->SetFont('Arial','',10);  //$pdf->Cell(50);

        $pdf->Cell(30, 6, $elem->expense_type, 0, 0, '', 1);
        $pdf->Cell(47, 6, "$".number_format($elem->total_amount,2), 0, 0,'', 1);
        $pdf->Cell(40, 6, "$".number_format($subtotal,2), 0, 0, '', 1);
...
$file_name = "../../../../storage/beestock/entities/sentpayment/sent-invoice/f-sent_id-".rand ( 89 , 9999999 ).".pdf";
