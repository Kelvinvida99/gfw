        ...
        foreach ($opcion as $key => $value) {
            if ($value->item_id == $elem['item_id'] && $value->purchase_item_id== $elem['id']) {
                ...
                for ($i = 1; $i <= $value->cant; $i++) {
                    ...
                    $pdf->Rect(1, 42, 98.2, 20);
                    $pdf->SetXY(1, 50);
                    $pdf->SetFont('Arial', 'B', 18);
                    $pdf->Cell(98.2, 8, substr($elem['item_name'], 0, 20), 0, 0, 'C');

                    $pdf->SetXY(1, 43);
                    $pdf->SetFont('Arial', 'B', 14);
                    $pdf->Cell(98.2, 8, substr($elem['brand'], 0, 30), 0, 0, 'C');
                    ...
                    $pdf->SetXY(1, 73);
                    $pdf->SetFont('Arial', '', 7);
                   // $pdf->Cell(50, 8, "Shelf Life", 0, 0, 'C');
                    //$pdf->Cell(50, 8, "Temperature", 0, 0, 'C');

                    $pdf->SetXY(1, 78);
                    $pdf->SetFont('Arial', 'B', 14);
                    //$pdf->Cell(50, 8, $elem['shelf_life'], 0, 0, 'C');
                    //$pdf->Cell(50, 8, $elem['temperature'], 0, 0, 'C');
                    ...
                    //$pdf->Cell(98.2, 8, "BARRA", 0, 0, 'C');

                    /*if (isset($rows[0]["code"])) {
                        $pdf->SetFillColor(0,0,0);
                        $pdf->Code128(10,270,$rows[0]["code"],55,16);
                       // $pdf->SetXY(10,286);
                        $pdf->Write(5,$rows[0]["code"]);
