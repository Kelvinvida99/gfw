...
$limit1            = 0; 
$limit2            = 5;
$mainFilter        = json_decode('{"fields":["patients.id","__name" ],"values":["",""]}');
...
//writeToTestTxt($selectAllRows); exit;
//echo  "<br>COUNT: ".$allRowsCountSql; 


//EXECUTING SQL QUERY TO GET ROWS
 if (!$result = mysqli_query($con, $selectAllRows)){
    echoReturnObject("MySqlError2");
    exit;
  }
  $rows = [];
...
//EXECUTING SQL QUERY TO GET ROWS QTY WITH CURRENT FILTER
 if (!$result = mysqli_query($con, $allRowsCountSql)) {
    echoReturnObject("MySqlError4");
    exit;
  }
