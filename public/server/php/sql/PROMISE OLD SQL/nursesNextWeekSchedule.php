...
WHERE services.deleted = '0'  AND  nurses_vs_agreements.nursesId = '$NURSE_ID' AND services.N_end_date >= '$nextMonday'  AND services.N_end_date <= '$nextSunday'  GROUP BY services.id   ";
      ...
      if($colNames[$c] == "statusHistory"){
       ...
       // if(sizeof($array) > 1){ 
          foreach ($array as $a) {
            ...
            if($tr == "[" && sizeof($b) > 1){
             // print_r( $b); echo "***<br>";
              $tr .= '{"id":'.json_encode($b[3]).',"time":'.json_encode($b[0]).', "status":'.json_encode($b[1]).',"notes":'.json_encode($b[2]).'}';  
              //print_r( $b); echo "***<br>";
            }
            elseif(sizeof($b) > 1)
              $tr .= ',{"id":'.json_encode($b[3]).',"time":'.json_encode($b[0]).', "status":'.json_encode($b[1]).',"notes":'.json_encode($b[2]).'}'; 
      ...
      elseif($colNames[$c] == "clockVsTasks"){
        ...
        foreach ($array as $a) {
         ...
         // if(sizeof($b)>1){//TO AVOID UNDIFINED OFFSET FOR TASKS 
          if($tr == "[" && sizeof($b) > 1)            
             $tr .= '{"id":'.json_encode($b[0]).', "N_name":'.json_encode($b[1]).',"N_input_helper":'.json_encode($b[2]).',"N_data_type":'.json_encode($b[3]).',"N_value":'.json_encode($b[4]).',"N_time":'.json_encode($b[5]).',"N_descr":'.json_encode($b[6]).',"N_mandatory":'.json_encode($b[7]).',"N_category":'.json_encode($b[8]).',"N_subCategory":'.json_encode($b[9]).'}';  
          elseif(sizeof($b) > 1)
          $tr .= ',{"id":'.json_encode($b[0]).', "N_name":'.json_encode($b[1]).',"N_input_helper":'.json_encode($b[2]).',"N_data_type":'.json_encode($b[3]).',"N_value":'.json_encode($b[4]).',"N_time":'.json_encode($b[5]).',"N_descr":'.json_encode($b[6]).',"N_mandatory":'.json_encode($b[7]).',"N_category":'.json_encode($b[8]).',"N_subCategory":'.json_encode($b[9]).'}';
     // }
        }
