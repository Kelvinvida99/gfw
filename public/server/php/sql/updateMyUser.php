...
/*
$id                = $_SESSION["user"]->userId; 
$fields            = json_decode("[\"email\"]");
$values            = json_decode("[\"carolinaCruda\"]");
    ...
    foreach (array_combine($fields, $values) as $field => $value) {
        if($field == "__password"){
            ...
            if($value != "" && $value != "UserHasPassword1!"){
                ...
                $newPass = $value; 
                $oldPass = $values[array_search("__oldPassword", $fields)];
                writeToTestTxt($oldPass);// exit;
               $rrr =  changeMyPassword($con, $newPass, $oldPass);
            }
            //else DO NOTHING, IF PASSWORD == "" OR PASSWORD == "UserHasPassword1!"  MEANS NOT NEED TO EDIT

        }elseif($field == "username" || $field == "__oldPassword"){
...
//writeToTestTxt(implode(" ",$mainUpdateSql)); exit;
...
if($allQueiresArray != []){
    ...
    if(!runQueries($allQueiresArray, $con)){
        echoReturnObject("MySqlError1", mysqli_error($con));
        exit;
    }
}
...
if (!$result = mysqli_query($con, $selectRow)){
    echoReturnObject("MySqlError2", mysqli_error($con));
    exit;
  }
  $rows = [];
